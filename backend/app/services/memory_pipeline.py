...
from app.models.memory import Location
...
class MemoryPipelineService:
    ...
    async def _resolve_location(self, user_id: int, loc_data):
        # Resolve House
        house_name = loc_data.house_name
        house = None
        if house_name:
            house = await self.repo.get_house_by_name(user_id, house_name)
            if not house:
                house = await self.repo.create_house(user_id, house_name)
        else:
            # Try to get the first house if not specified
            houses = await self.repo.get_houses(user_id)
            if houses:
                house = houses[0]
            else:
                # Last resort: Create "Main House" if user has no houses
                house = await self.repo.create_house(user_id, "Main House")

        # Resolve Room
        room = await self.repo.get_room_by_name(user_id, loc_data.room, house_id=house.id)
        if not room:
            room = await self.repo.create_room(user_id, loc_data.room, house_id=house.id)
    ...
    def _format_location(self, location: Location):
        ...
        # In async context, relationships should be pre-loaded.
        # We check if they are loaded to avoid MissingGreenlet error.
        try:
            if location.room:
                if location.room.house:
                    parts.append(location.room.house.name)
                parts.append(location.room.name)
            ...
            return f"Location(id={location.id})"
